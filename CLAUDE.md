...
- `environments/<env>/` — root modules, one per AWS account. Each wires together the shared modules and holds the backend config, provider/assume-role config, and tfvars for that account.
  - `noprod/` — implemented; shared by dev and uat (both run in the same non-prod AWS account, e.g. the ECS cluster `simulador-nonprod` is shared between them).
...
- `modules/` — reusable modules consumed by the environment roots.
  ...
  - `vpc-endpoints/` — Gateway endpoints (S3, DynamoDB) always created (free); Interface endpoints (ECR, Secrets Manager, etc.) are opt-in via `interface_endpoint_services`, since they incur hourly + per-GB cost.
  - `ecs-cluster/` — ECS cluster with FARGATE + FARGATE_SPOT capacity providers. Spot is weighted by default (favors cost for interruption-tolerant dev/uat workloads); prod should override `default_capacity_provider_weight_spot = 0` via tfvars if the extraction engine must not run on Spot.
  - `foundation/` — not yet implemented (empty directory).

## Commands

Run from inside an environment directory (e.g. `environments/noprod`):
...
- When adding a new environment root, mirror `environments/noprod/`'s file split: `backend.tf` (remote state), `provider.tf` (assume-role + default tags), `variables.tf`, `terraform.tfvars`, `main.tf` (module calls + any root-level resources), `output.tf`.
